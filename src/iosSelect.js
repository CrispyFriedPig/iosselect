...
(function() {
  ...
  function IosSelect(level, data, options) {
    ...
    // this.options.oneLevelContainBG = options.oneLevelContainBG || 'transparent';
  ...
  IosSelect.prototype = {
    ...
    initLayer: function() {
      ...
      var all_html = [
        ...
        // '<hr class="cover-area2"/>',
          // 新增固定浅黄背景
        // '<div class="selected-cover-box" id="selectedCoverBox">\'',
      ...
      // 新增容器背景
      // if (this.options.headerHeight && this.options.itemHeight) {
      //     this.oneLevelContain = this.iosSelectLayer.el.querySelector('.one-level-contain');
      //     this.oneLevelContain.style.paddingTop = this.options.itemHeight * this.options.coverArea1Top + this.options.cssUnit;
      //     this.oneLevelContain.style.paddingBottom = this.options.itemHeight * this.options.coverArea1Top + this.options.cssUnit;
      //     this.oneLevelContain.style.backgroundColor = this.options.oneLevelContainBG;
      //     this.oneLevelContain.style.backgroundClip = 'padding-box';
      // }
    // <div class="one-level-contain" id="oneLevelContain">'
      ...
      // 浅黄背景
      // this.selectedCoverBoxDom = this.iosSelectLayer.el.querySelector('.selected-cover-box');
      // this.selectedCoverBoxDom.style.top = this.options.headerHeight + this.options.itemHeight * this.options.coverArea1Top + this.options.cssUnit;
      // this.selectedCoverBoxDom.style.height = this.options.itemHeight + this.options.cssUnit;
      ...
      this.scrollOne.on('scrollEnd', function() {
        ...
        self.toggleClassList(self.oneLevelContainDom,1);

        var pdom = self.changeClassName(self.oneLevelContainDom, plast,1);
    ...
    renderOneLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, oneLevelData) {
      ...
      oneLevelData.forEach(function(v, i, o) {
        if (v.id == oneLevelId) {
          // oneHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';" ' + iosSelectUtil.attrToHtml(v) + ' class="at"><marquee /*behavior="alternate"*/ scrollamount=3 direction="left" align="middle">' + v.value + '</marquee></li>';

          var styleStr = ''
          // if(v.value.length>30){
          //   var basetime = 10;
          //   var extraTime = v.value.length/3;
          //   var totalTime = Math.ceil(basetime+extraTime);
          //   styleStr +=
          //     '-webkit-animation:marquee ' + totalTime + 's linear infinite;' +
          ...
          // pdom.style.animation = 'marquee ' + totalTime + 's linear infinite';
          // pdom.style.webkitAnimation = 'marquee ' + totalTime + 's linear infinite';
          // pdom.style.animationDelay = '3s';
          // pdom.style.webkitAnimationDelay = '3s';

          oneHtml += '<li style="' + styleStr + 'height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';" ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
          plast = i + 1;
        } else {
      ...
      if (pdom.offsetWidth<=pdom.parentElement.clientWidth){
        ...
        var totalTime = Math.ceil(basetime+extraTime);
        // pdom.style.margin='unset !important';

        // 动画轨迹运算
        var keyframesName = 'marquee141-150';
        var overFlowWidthRatio = (pdom.offsetWidth - pdom.parentElement.clientWidth)/pdom.offsetWidth;
        if(overFlowWidthRatio<=0.1){
          keyframesName = 'marquee101-110';
        } else if(overFlowWidthRatio<=0.2){
          keyframesName = 'marquee111-120';
        } else if(overFlowWidthRatio<=0.3){
          keyframesName = 'marquee121-130';
        } else if(overFlowWidthRatio<=0.4){
          keyframesName = 'marquee131-140';
        } else if(overFlowWidthRatio<=0.5){
          keyframesName = 'marquee141-150';
        }

        pdom.style.animation = keyframesName + ' ' + totalTime + 's linear infinite';
    ...
    changeClassName: function(levelContainDom, plast) {
      ...
      if (end) {
        // 增加跑马灯标签 start
        // var tempContent = pdom.textContent;
        // pdom.textContent = "";
        // var marqueeDom = document.createElement("marquee");
        // marqueeDom.textContent = tempContent;
        // marqueeDom.setAttribute('align','middle');
      ...
      if (end) {
        Array.prototype.slice.call(dom.querySelectorAll('li')).forEach(function (v) {
          ...
          if (v.classList.contains('at')) {
            ...
            v.style.animation = 'unset';
            v.style.webkitAnimation = 'unset';
            v.style.animationDelay = 'unset';
            v.style.webkitAnimationDelay = 'unset';
